...
const CompareSlider = ({
    ...
    return (
        <div
            ref={screenRef}
            className={`${afterOnly ? '' : 'cursor-ew-resize'} select-none overflow-hidden ${className}`}
            style={style}
            onMouseDown={afterOnly ? undefined : (e) => { isDragging.current = true; updatePos(e.clientX) }}
            onTouchStart={afterOnly ? undefined : (e) => { isDragging.current = true; updatePos(e.touches[0].clientX) }}
        >
